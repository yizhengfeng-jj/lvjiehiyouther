...
define(function(require, exports, module) {
    ...
    _public.dom = function() {
        ...
        //    }
        //});
        var name = common.parseURL(window.location.href).params.name;
        var id = common.parseURL(window.location.href).params.id;

        $(".box-header  .title").text("商家认证-" + decodeURI(name));
        ...
        $('#p-submit').click(function() {
            ...
            //    return ;
            //}
            if($("#h1-upload-img4").attr("src").trim()==""){
                jalert.error("您还没有上传营业执照");
                return ;
            }

            //common.send('subidval', {idcard_num: idcard, realname: name, type: 'all'}).done(function(res) {
            //    if(res.code == 1) alert('已提交信息')
