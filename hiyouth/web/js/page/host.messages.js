...
define(function(require, exports, module) {
    ...
    //start
    _public.tabRender = function(container, tpl, url, type,id) {
        var $all = container;
    ...
    _public.bindEvent = function() {
        ...
        $(".tab").on("click", function(e) {
            ...
            if ($this.text().indexOf("评论") != -1) {
              //  var tmpid=common.parseURL(window.location.href).params.id;

                _public.tabRender($("#comment"), "comment-tpl", "getBcomment", 1);


            } else if ($this.text().indexOf("问答") != -1) {

                _public.tabRender($("#question"), "question-tpl", "getBcomment", 0);
    ...
    _public.dom = function() {
        ...
        //系统消息一开始就设置
        var cu = parseInt($(".message-new-vk").text());
        if(cu>0){
            $("#delegate .tab-new:eq(2)").text("+"+cu).fadeIn();
        }
        ...
        $(".card-list").on("click", "a", function(e) {
            ...
            var question  = $this.hasClass("question");

            var $li = $this.parents("li");
            if (hotel || recruit || question) {
              ...
              common.sendPost("deleteAllPush", sendData).done(function(result) {
        ...
        function reply($a) {
            ...
            $div.on("click", "a", function() {
                ...
                if (reply) {
                    ...
                    var rep_type = ($ul.parent().attr("id") == "comment" ? "评论" : "问答");//评论是1 问答是0
